...
class cfs_test_group_cfs_ci_lab(Group):
    ...
    def test_aliveness(self):
        ...
        Group.print(f"Testing CI_LAB aliveness on <%= target_name %>")

        # Verify that we have a recent packet (by waiting for a new one to arrive)
        wait_check_packet(f"<%= target_name %>", f"CI_LAB_HK", 1, 100)

        # Assuming no one else is sending commands, grab the latest command count
        cmd_count = tlm(f"<%= target_name %> CI_LAB_HK COMMAND_COUNTER")

        # Check accepted NOOP command proving application is up and running
        cmd(f"<%= target_name %> CI_LAB_CMD_NOOP")
        wait_check(f"<%= target_name %> CI_LAB_HK COMMAND_COUNTER == {cmd_count + 1}", 100)

        # Check accepted Reset Counters command
        cmd(f"<%= target_name %> CI_LAB_CMD_RESET_COUNTERS")
        wait_check(f"<%= target_name %> CI_LAB_HK COMMAND_COUNTER == 0", 100)
