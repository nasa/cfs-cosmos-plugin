...
class cfs_test_group_cfs_hs(Group):
    ...
    def test_application_monitor(self):
        ...
        cmd_count = self.util_setup_test_command(app_name, test_cmd)

        # Get current state value
        cmd_state = tlm(f"<%= target_name %> {app_name}_HK APP_MON_STATE")

        # Check accepted Enable Application Monitor Command and verify command counter
        cmd(f"<%= target_name %> {app_name}_CMD_ENA_APP_MON")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Application Monitor telemetry
        wait_check(f"<%= target_name %> {app_name}_HK APP_MON_STATE == 'ENABLED'", 100)
        ...
        self.util_verify_event_message(app_name, test_cmd, event_id, event_msg)
        ...
        # Check accepted Enable Application Monitor Command and verify command counter
        cmd(f"<%= target_name %> {app_name}_CMD_DIS_APP_MON")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Application Monitor telemetry
        wait_check(f"<%= target_name %> {app_name}_HK APP_MON_STATE == 'DISABLED'", 100)
    ...
    def test_events_monitor(self):
        ...
        # Get current state value
        cmd_state = tlm(f"<%= target_name %> {app_name}_HK EVENT_MON_STATE")

        # Check accepted Enable Events Monitor Command and verify command counter
        cmd(f"<%= target_name %> {app_name}_CMD_ENA_EVENT_MON")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Event Monitor telemetry
        wait_check(f"<%= target_name %> {app_name}_HK EVENT_MON_STATE == 'ENABLED'", 100)
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_DIS_EVENT_MON")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Event Monitor telemetry
        wait_check(f"<%= target_name %> {app_name}_HK EVENT_MON_STATE == 'DISABLED'", 100)
    ...
    def test_aliveness_indicator(self):
        ...
        # Get current state value
        cmd_state = tlm(f"<%= target_name %> {app_name}_HK ALIVENESS_STATE")

        # Check accepted Enable Aliveness Indicator Command and verify command counter
        cmd(f"<%= target_name %> {app_name}_CMD_ENA_ALIVENESS")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Aliveness Indicator telemetry
        wait_check(f"<%= target_name %> {app_name}_HK ALIVENESS_STATE == 'ENABLED'", 100)
        ...
        # Check accepted Enable Aliveness Indicator Command and verify command counter
        cmd(f"<%= target_name %> {app_name}_CMD_DIS_ALIVENESS")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Aliveness Indicator telemetry
        wait_check(f"<%= target_name %> {app_name}_HK ALIVENESS_STATE == 'DISABLED'", 100)
    ...
    def test_reset_processor_resets_performed_count(self):
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_CLR_CPU_RESET_CNT")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Reset Processor Resets Performed telemetry
        wait_check(f"<%= target_name %> {app_name}_HK CPU_RESET_CNT == 0", 100)
    ...
    def test_set_max_processor_resets(self):
        ...
        max_resets = tlm(f"<%= target_name %> {app_name}_HK MAX_RESETS")

        # Check accepted Set Max Processor Resets Command and verify command counter
        cmd_resets = 1024
        cmd(f"<%= target_name %> {app_name}_CMD_SET_MAX_CPU_RESETS with MAX_RESETS {cmd_resets}, PADDING 0")
        self.util_verify_command_count(app_name, cmd_count)

        # Check Set Max Processor Resets telemetry
        wait_check(f"<%= target_name %> {app_name}_HK MAX_RESETS == {cmd_resets}", 100)
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_SET_MAX_CPU_RESETS with MAX_RESETS {max_resets}, PADDING 0")
    ...
    def test_cpu_hogging_indicator(self):
        ...
        # Get current state value
        cmd_state = tlm(f"<%= target_name %> {app_name}_HK CPU_HOG_STATE")

        # Check accepted Enable CPU Hogging Indicator Command and verify command counter
        cmd(f"<%= target_name %> {app_name}_CMD_ENA_CPU_HOG")
        self.util_verify_command_count(app_name, cmd_count)

        # Check CPU Hogging Indicator telemetry
        wait_check(f"<%= target_name %> {app_name}_HK CPU_HOG_STATE == 'ENABLED'", 100)
        ...
        # Check accepted Enable CPU Hogging Indicator Command and verify command counter
        cmd(f"<%= target_name %> {app_name}_CMD_DIS_CPU_HOG")
        self.util_verify_command_count(app_name, cmd_count)

        # Check CPU Hogging Indicator telemetry
        wait_check(f"<%= target_name %> {app_name}_HK CPU_HOG_STATE == 'DISABLED'", 100)
