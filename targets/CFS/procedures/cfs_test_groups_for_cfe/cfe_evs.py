...
class cfs_test_group_cfe_evs(Group):
    ...
    def test_01_event_type(self):
        ...
        Group.print(f"Testing {app_name} Disable/Enable Event Type on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_ENABLE_EVENT_TYPE with BIT_MASK DEBUG")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
    ...
    def test_02_set_event_format_mode(self):
        ...
        Group.print(f"Testing {app_name} Set Event Format Mode on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_SET_EVENT_FORMAT_MODE with MSG_FORMAT SHORT")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
    ...
    def test_03_app_event_type(self):
        ...
        Group.print(f"Testing {app_name} Enable/Disable App Event Type on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_ENABLE_APP_EVENT_TYPE with APP_NAME SAMPLE_APP, BIT_MASK DEBUG")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
    ...
    def test_04_app_events(self):
        ...
        Group.print(f"Testing {app_name} Enable/Disable App Events on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_DISABLE_APP_EVENTS with APP_NAME SAMPLE_APP")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
    ...
    def test_05_reset_app_counter(self):
        ...
        Group.print(f"Testing {app_name} Reset App Counter on <%= target_name %>")
    ...
    def test_06_filter_commands(self):
        ...
        Group.print(f"Testing {app_name} Filter Commands on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_ADD_EVENT_FILTER with APP_NAME SAMPLE_APP, EVENT_ID 3, MASK 0x0001")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)

        # Check accepted Set Filter command
        cmd_count = tlm(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER")
        cmd(f"<%= target_name %> {app_name}_CMD_SET_FILTER with APP_NAME SAMPLE_APP, EVENT_ID 3, MASK 0x0003")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)

        # Check accepted Reset Filter command
        cmd_count = tlm(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER")
        cmd(f"<%= target_name %> {app_name}_CMD_RESET_FILTER with APP_NAME SAMPLE_APP, EVENT_ID 3")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)

        # Check accepted Delete Event Filter command
        cmd_count = tlm(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER")
        cmd(f"<%= target_name %> {app_name}_CMD_DELETE_EVENT_FILTER with APP_NAME SAMPLE_APP, EVENT_ID 3")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
    ...
    def test_07_ports(self):
        ...
        Group.print(f"Testing {app_name} Enable/Disable Ports on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_ENABLE_PORTS with BIT_MASK 2")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
    ...
    def test_08_write_app_data_file(self):
        ...
        Group.print(f"Testing {app_name} Write App Data Files on <%= target_name %>")
    ...
    def test_09_write_log_data_file(self):
        ...
        Group.print(f"Testing {app_name} Write Log Data File on <%= target_name %>")
    ...
    def test_10_set_log_mode(self):
        ...
        Group.print(f"Testing {app_name} Set Log Mode on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_SET_LOG_MODE with LOG_MODE OVERWRITE")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
    ...
    def test_11_clear_log(self):
        ...
        Group.print(f"Testing {app_name} Clear Log on <%= target_name %>")
        ...
        cmd(f"<%= target_name %> {app_name}_CMD_CLEAR_LOG")
        wait_check(f"<%= target_name %> {app_name}_HK COMMAND_COUNTER == {cmd_count + 1}", 100)
